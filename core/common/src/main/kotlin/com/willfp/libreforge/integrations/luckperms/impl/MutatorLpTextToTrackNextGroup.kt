...
object MutatorLpTextToTrackNextGroup : Mutator<NoCompileData>("lp_text_to_track_next_group") {
    override val description = "Sets the text to the group above the primary group of the player in a LuckPerms track."

    override val categories = setOf("chat", "permission")

    override val additionalInfo = listOf(
        "Requires the LuckPerms plugin.",
        "No-ops if the track is not loaded, or if the player is already in the last group of the track."
    )
    ...
    override fun mutate(data: TriggerData, config: Config, compileData: NoCompileData): TriggerData {
        ...
        return data.copy(
            text = track.getNext(group) ?: return data
        )
    }
}
