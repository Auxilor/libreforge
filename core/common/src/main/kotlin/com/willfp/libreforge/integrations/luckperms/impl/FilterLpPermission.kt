...
object FilterLpPermission : Filter<NoCompileData, Collection<String>>("lp_permission") {
    override val description = "Matches when the LuckPerms node key in the trigger is one of the given keys."
    override val categories = setOf("permission")
    override val valueType = ArgType.STRING_LIST
    override val additionalInfo = listOf(
        "Requires the LuckPerms plugin.",
        "Matches against raw node keys, so a group node is group.vip rather than vip.",
    ...
    override fun isMet(data: TriggerData, value: Collection<String>, compileData: NoCompileData): Boolean {
        ...
        return value.containsIgnoreCase(node.key)
    }
}
