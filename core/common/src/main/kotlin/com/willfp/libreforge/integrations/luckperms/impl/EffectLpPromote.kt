...
object EffectLpPromote : Effect<NoCompileData>("lp_promote") {
    override val description = "Promotes the player along a LuckPerms track."
    override val categories = setOf("permission")
    override val additionalInfo = listOf(
        "Requires the LuckPerms plugin.",
        "Saves asynchronously, so the change is not visible immediately.",
        "Does nothing if the track is not loaded, or if the player is already in the last group of the track."
    )
    ...
    override val arguments = arguments {
        require(
            ...
            description = "The name of the LuckPerms track to promote along.",
            type = ArgType.STRING,
            example = "ranks"
        )
        optional(
            "contexts",
            description = "The LuckPerms contexts to promote in, as a list of key=value entries.",
