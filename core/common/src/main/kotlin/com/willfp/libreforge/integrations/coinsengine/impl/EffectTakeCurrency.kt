...
object EffectTakeCurrency : Effect<String?>("take_currency") {
    override val parameters = setOf(
        TriggerParameter.PLAYER
    )

    override val arguments = arguments {
        require("currency", "You must specify the currency to take!")
        require("amount", "You must specify the amount of currency to take!")
    }
    ...
    override fun onTrigger(config: Config, data: TriggerData, compileData: String?): Boolean {
        ...
        CoinsEngineAPI.getCurrency(currency)?.let {
            return CoinsEngineAPI.removeBalance(uuid, it, amount)
        }
    ...
    override fun makeCompileData(config: Config, context: ViolationContext): String? {
        ...
        if (currency !in validCurrencies) {
            context.log(
                ...
                ConfigViolation(
                    "currency",
                    "You must specify the currency to take! Valid currencies are: ${validCurrencies.joinToString(", ")}"
                )
            )
            return null
        }
